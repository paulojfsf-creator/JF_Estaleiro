...
class TestViaturaManutencao:
    ...
    def test_get_viaturas_list(self):
        ...
        response = requests.get(f"{BASE_URL}/api/viaturas", headers=self.headers)
    ...
    def test_create_test_viatura(self):
        ...
        if not test_viatura:
            # Create test viatura
            response = requests.post(f"{BASE_URL}/api/viaturas", headers=self.headers, json={
        ...
        self.test_viatura_id = test_viatura.get("id")
        return test_viatura
    ...
    def test_get_viatura_detail_with_maintenance(self):
        ...
        # Get detail
        response = requests.get(f"{BASE_URL}/api/viaturas/{viatura_id}", headers=self.headers)
        assert response.status_code == 200
    ...
    def test_cleanup_test_viatura(self):
        ...
        if test_viatura:
            viatura_id = test_viatura.get("id")
            response = requests.delete(f"{BASE_URL}/api/viaturas/{viatura_id}", headers=self.headers)
            assert response.status_code == 200
